...
\begin{itemize}
  \item name\_table.txt - spreadsheet containing the analyzed data and
    ...
    \item name\_roc.pdf - true vs false positivity curves of the
...
@ %def
\normalsize
User can plot the roc curves directly by (figure $1$)
\scriptsize
@
<<eval=false>>=
