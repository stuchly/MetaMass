...
3. Copy contents in supplementary Table 1.11, (Annotation Table R-version) . Copy data into new spreadsheet.  Save as tab-delimited text in folder named e.g. Test, name the file MyMarkers.txt
...
“Christoforou”), out [tab], “Fig2a”)
\end{tip}
Explanation: analyze.MSfile: name of function used to perform all the analyses.
MSfile: dataset to be analyzed,
Metadata (see \texttt{?Metadata}): The “Christoforou” dataset is one
of several stored in MetaMass as reference data. A complete list is
found in supplementary Table 1.10.  The meta-data is not clustered or
used for the classification, but simply aligned to the processed test
dataset as reference.
Overlap=2 . The analysis is restricted to
proteins that are identified both in the dataset and the metadata
(\textcolor{red}{Edit: the behavior has changed in current
  version. The \texttt{overlap} parameter affects now only data used
  for clustering i.e. the presence of protein in Metadata is not
  checked unless \texttt{cluster.metdata=TRUE}. For desired behavior
  set \texttt{overlap=NULL} which is default - see \hyperref[ex2]{Example 2}}
Clusters: number of clusters to generate in K-means clustering. (an
average cluster size of 5 is recommended which is default)
...
Fig2a.cdt file is a heatmap  file (JavaTreeView), The algorithm generates a single heatmap corresponding to the first of the marker sets, in this case marker set 3. (Uniprot/GO overlap + study 1).
...
@ %def
\normalsize
The results are stored in the working directory in files starting with 'Fig2a'.
...
Analyze dataset1, use dataset 4,9 and 10, as Metadata.
\scriptsize
@
<<>>=
...
\label{ex5}
As another example let us show how to use multiple annotations (one can use custom annotation file if
...
By default only proteins which were detected in all studies (groups)
and Metadata are used for clustering. This could be bypassed by
setting the parameter \texttt{overlap=$k$} - in this case missing
values are replaced by $1$s and only the
proteins which were detected in at least $k$ studies are used for
clustering. The other proteins would remain in the table and heatmap
as unassigned.
\scriptsize
@
<<>>=
...
In this case of course we leave large proportion of proteins
unclassified see figure \ref{roc2}.
<<produce-plot, results = hide,echo=false>>=
pdf('roc2.pdf')
par(mfrow=c(3,3),mar=c(1, 4, 2.2, 1) + 0.1,cex=0.45)
