...
MyMarkers<-system.file("extdata","MyMarkers.txt",package="MetaMass")
analyze.MSfile(MSfile = file1, clusters=432, Metadata = c("Carvalho", "Bileck", "Thakar"), Annotation = MyMarkers, markers=c(3,4,6,7,9), output = "study1_MyMark")
@ %def
\normalsize
Analyze dataset 4,9 and 10, use dataset1 as Metadata
\scriptsize
...
<<>>=
files1<-system.file("extdata",c("Bileck.txt","Thakar.txt","Carvalho.txt"),package="MetaMass")
analyze.MSfile(MSfile = files1,clusters = 473,Annotation = MyMarkers.txt, markers=c(3,4,6,7,9),output = "study4_9_10_MyMark")
@ %def
\normalsize
